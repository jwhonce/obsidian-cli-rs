...
Content"#;
        ...
        assert!(!frontmatter.is_empty());
        assert!(frontmatter["single_quoted"]
            .as_str()
            .unwrap()
            .contains("double quotes"));
        assert!(frontmatter["double_quoted"]
            .as_str()
            .unwrap()
            .contains("single quotes"));
        assert!(frontmatter["mixed_quotes"]
            .as_str()
            .unwrap()
            .contains("escaped quotes"));
        assert!(frontmatter["special_yaml"]
            .as_str()
            .unwrap()
            .contains("YAML:"));
    }
...
---"#;
    ...
    fn test_serialize_with_complex_values() {
        ...
        frontmatter.insert(
            "title".to_string(),
            Value::String("Complex Test".to_string()),
        );
        ...
        frontmatter.insert("numbers".to_string(), json!([1, 2.5, -4.25]));
...
Original content here"#;
        ...
        let (roundtrip_frontmatter, roundtrip_body) =
            frontmatter::parse_string(&serialized).unwrap();
...
Original content"#,
    ...
    fn test_update_frontmatter_complex_values() {
        ...
        let result =
            frontmatter::update_frontmatter(&test_file, "complex_data", complex_value.clone());
        assert!(result.is_ok());
